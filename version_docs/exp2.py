...
def display_additional_information(selected_stock, selected_options, start_date=None, end_date=None):
    ...
    # If selected_options is a JSON string, parse it
    ...
    # selected_options should now be a dictionary
    ...
    if "Stock Actions" in selected_options and selected_options["Stock Actions"]:
    ...
    for option, checked in selected_options.items():
        if checked and option != "Stock Actions":  # Avoid redundant display for Stock Actions
            st.subheader(f"{selected_stock} - {option}")
            if option == "Quarterly Financials":
