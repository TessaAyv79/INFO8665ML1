...
def handle_analysis(selected_stock, analysis_type, start_date, end_date):
    # Önceden belirttiğiniz diğer işlemleri burada gerçekleştirin (yorum olarak belirtilmiş)

    # analysis_type'ın JSON olup olmadığını kontrol etme
    if isinstance(analysis_type, str):
        st.write(f"Received analysis_type: {analysis_type}")  # Debugging için

        # JSON olup olmadığını kontrol etme
        if analysis_type.startswith('{') and analysis_type.endswith('}'):
            try:
                analysis_type = json.loads(analysis_type)
                st.write("Successfully decoded JSON string for analysis_type")  # Başarılı JSON ayrıştırma mesajı
            except json.JSONDecodeError as e:
                st.error(f"Error decoding JSON string for analysis_type: {e}")
                return  # JSON çözümleme başarısız olursa fonksiyondan çık
        else:
            st.error("analysis_type is not in JSON format")
            return  # JSON formatında değilse fonksiyondan çık

    # Eğer analysis_type 'Predicted Prices' değilse, genel stok analizini ve ek bilgileri görüntüleyin
    if analysis_type != "Predicted Prices":
        display_stock_analysis(selected_stock, analysis_type, start_date, end_date)
        
        # display_additional_information fonksiyonunun doğru parametrelerle çağrılması
        if isinstance(analysis_type, dict):
            display_additional_information(selected_stock, analysis_type, start_date, end_date)
        else:
            st.error("analysis_type is not a dictionary, cannot proceed.")
    else:
        # 'Predicted Prices' ise tahmin edilen fiyatları görüntüleyin
...
def display_additional_information(selected_stock, analysis_type, start_date, end_date):
