...
# Centering the main image using HTML and CSS
image_path = r"C:\Users\Admin\Documents\MLAI\INFO8665ML\project2\image\images2.jpg"
image_path = image_path.replace("\\", "/")  # Replace backslashes with forward slashes
st.markdown(
    f"""
    <div style="position: absolute; top: 50%; left: 30%; transform: translate(-50%, -50%); text-align: center; color: white; font-size: 24px; font-weight: bold;">
...
# Display the image using st.image with controlled width
st.image(image_path, width=500)  # Adjust the width as needed
# Centered text overlay using HTML directly
...
n_years = st.sidebar('Years of prediction:', 1, 10)
period = n_years * 365
start_datex = start_date - timedelta(days=period)
...
start_date = st.sidebar.date_input("Start Date", n_years)
