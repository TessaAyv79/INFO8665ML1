...
def handle_analysis(selected_stock, analysis_type, start_date, end_date):
    if analysis_type != "Predicted Prices":
        ...
        display_additional_information(selected_stock, analysis_type, start_date, end_date)
    else:
...
def display_additional_information(selected_stock, selected_options, start_date, end_date):
    stock_data = prepare_data(selected_stock, start_date, end_date)
    ...
    # Ensure selected_options is a dictionary
    if isinstance(selected_options, str):
        # This could be a JSON string that needs parsing
        try:
            selected_options = json.loads(selected_options)
        except json.JSONDecodeError:
            st.error("Error decoding JSON string for selected_options")
            return  # Exit function if decoding fails

    # Now safely iterate if selected_options is a dictionary
    if isinstance(selected_options, dict):
        for option, checked in selected_options.items():
            if checked:
                st.write(f"Option: {option} is checked.")
            else:
                st.write(f"Option: {option} is not checked.")
    else:
        st.error("selected_options is not a dictionary, cannot iterate.")

    # Display stock actions as an example
