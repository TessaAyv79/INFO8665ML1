...
def display_additional_information(selected_stock, selected_options, start_date, end_date):
    ...
    # Instantiate the stock object once
    stock = yf.Ticker(selected_stock)

    # Display stock actions if any
    st.subheader(f"{selected_stock} - Stock Actions")
    stock_actions = stock.actions  # Correctly retrieve stock actions
    if not stock_actions.empty:
        st.write(stock_actions)  # Displaying all actions such as dividends and stock splits
    else:
        st.write("No data available for Stock Actions")

    # Iterate over options and display the corresponding information
