...
# Function to prepare data for LSTM model training and evaluation
...
def prepare_data(selected_stock, start_date, end_date):
    ...
    return df
# Function to train and evaluate RandomForestRegressor model
def train_and_evaluate_model(selected_stock, df):
    X = df[['Open', 'High', 'Low', 'Close', 'Volume']]  # Example features
    y = df['Close']  # Example target variable # 'Adj Close' yerine 'Close' kullanıyoruz, çünkü 'Adj Close' her zaman mevcut olmayabilir
...
def train_and_evaluate_lstm_model1(ticker, company_name, validate_period):
    
    df = prepare_data(ticker, validate_period)
...
def handle_analysis(selected_stock, analysis_type, start_date, end_date, TypeError: 'function' object is not subscriptable
Traceback:
File "C:\Users\Admin\Documents\MLAI\venv\CSCN8010_clascis_ml\Lib\site-packages\streamlit\runtime\scriptrunner\script_runner.py", line 600, in _run_script
    exec(code, module.__dict__)
File "C:\Users\Admin\Documents\MLAI\INFO8665ML\project2\exp4.py", line 794, in <module>
    handle_analysis(selected_stock, analysis_type, start_date, end_date, validate_period)
File "C:\Users\Admin\Documents\MLAI\INFO8665ML\project2\exp4.py", line 349, in handle_analysis
    display_additional_information(selected_stock, selected_options, validate_period)
File "C:\Users\Admin\Documents\MLAI\INFO8665ML\project2\exp4.py", line 451, in display_additional_information
    train_and_evaluate_model(selected_stock, df)
File "C:\Users\Admin\Documents\MLAI\INFO8665ML\project2\exp4.py", line 244, in train_and_evaluate_model
    X = df[['Open', 'High', 'Low', 'Close', 'Volume']]  # Example features
        ~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^):
    if analysis_type != "Predicted Prices":
        ...
        for selected_stock in selected_stocks:
            ...
            train_and_evaluate_model(selected_stock, df)
            train_and_evaluate_lstm_model1(selected_stock, df)
            train_and_evaluate_dnn_model(selected_stock, df)
...
def display_additional_information(selected_stock, selected_options, df):
...
def train_and_evaluate_dnn_model(ticker, df):
    df = prepare_data(ticker, df)
    X = df[['10_day_MA', '20_day_MA', '50_day_MA', 'Daily_Return']]
    y = df['Close']  # 'Adj Close' yerine 'Close' kullanıyoruz, çünkü 'Adj Close' her zaman mevcut olmayabilir
