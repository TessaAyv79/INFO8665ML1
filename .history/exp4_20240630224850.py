...
+        prd (str): The period of the stock data.
+
+    Returns:
+        pandas.DataFrame: The prepared data.
+
+    Raises:
+        ValueError: If no data is found for the ticker and period.
+        Exception: If an error occurs while preparing the data.
+    """
+    df = stock_data.history(period=prd)
-        df = stock_data.history(period=prd)
-        
-        if df.empty:
-            raise ValueError(f"No data found for {ticker} with period {prd}")
-        
-        df['10_day_MA'] = df['Close'].rolling(window=10).mean()
-        df['20_day_MA'] = df['Close'].rolling(window=20).mean()
-        df['50_day_MA'] = df['Close'].rolling(window=50).mean()
...
-        raise e
 def train_and_evaluate_model(ticker, company_name, prd):
+    """
+    Train and evaluate a model using prepared data.
+
+    Args:
+        ticker (str): The ticker symbol of the stock.
...
+        prd (str): The period of the stock data.
+    """
     df = prepare_data(ticker, prd)
     X = df[['10_day_MA', '20_day_MA', '50_day_MA', 'Daily_Return']]
+    y = df['Close']  # 'Adj Close' yerine 'Close' kullanıyı, çünkü 'Adj Close' her zaman mevcut olmayabilir
...
+    # Evaluate the models
-    # Model değerlendirme
<<<<<<<  914a8628-6482-40dc-865a-6a7309cc08b4  >>>>>>>
...
def train_and_evaluate_lstm_model1(ticker, company_name, prd):
        ...
        if checked:
            ...
            if option == "Stock Actions":
                ...
                for selected_stock in selected_stocks:
                    ...
                    train_and_evaluate_model(selected_stock, company_name, prd)
                    train_and_evaluate_lstm_model1(selected_stock, company_name, prd)
                    train_and_evaluate_dnn_model(selected_stock, company_name, prd)
# DNN modeli eğitimi ve değerlendirme fonksiyonu
def train_and_evaluate_dnn_model(ticker, company_name, prd):
    df = prepare_data(ticker, prd)
    X = df[['10_day_MA', '20_day_MA', '50_day_MA', 'Daily_Return']]
    y = df['Close']  # 'Adj Close' yerine 'Close' kullanıyoruz, çünkü 'Adj Close' her zaman mevcut olmayabilir
