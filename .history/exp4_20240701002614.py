...
def prepare_data(selected_stock, validate_period):
    try:
        stock_data = yf.Ticker(selected_stock)
        df = stock_data.history(period=validate_period)
        
        if df.empty:
            st.warning(f"No data found for {selected_stock} for the selected period.")
            return None
        ...
        st.error(f"Failed to prepare data for {selected_stock}. Error: {str(e)}")
        return None
