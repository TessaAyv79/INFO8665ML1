...
def get_predictions(ticker, start_date, end_date):
    url = 'http://127.0.0.1:8502/api/predict'
    payload = {
        'ticker': ticker,
...
def display_predicted_prices():
    ...
    if st.button("Get Predictions"):
        predictions = get_predictions(ticker, start_date, end_date)
        if 'status' in predictions and predictions['status'] == 'error':
            st.error(predictions['message'])
        else:
            dates = predictions['dates']
            predicted_prices = predictions['predictions']
            df_predictions = pd.DataFrame({'Date': dates, 'Predicted Price': predicted_prices})
            st.write(df_predictions)

            # Plot the data
            st.line_chart(df_predictions.set_index('Date'))
