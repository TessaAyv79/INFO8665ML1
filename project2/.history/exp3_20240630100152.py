...
def send_email(feedback_text, recipient_email):
    try:
        # Set up the MIME
        message = MIMEMultipart()
        message['From'] = "your_email@gmail.com"
        message['To'] = recipient_email
        message['Subject'] = "User Feedback from Streamlit App"

        # Add the feedback text to the email
        message.attach(MIMEText(feedback_text, 'plain'))

        # Create SMTP session for sending the mail
        session = smtplib.SMTP('smtp.gmail.com', 587)  # Use Gmail's SMTP server
        session.starttls()  # Enable security
        session.login("your_email@gmail.com", "your_password")  # Login with your email and password
        text = message.as_string()
        session.sendmail("your_email@gmail.com", recipient_email, text)
        session.quit()
