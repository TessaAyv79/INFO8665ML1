...
def send_local_email(feedback, user_email):
    sender_email = "nejlaayvazoglu@hotmail.com"  # Gönderici e-posta adresiniz
    receiver_email = "recipient_email@example.com"  # Alıcı e-posta adresi

    # Mesajı oluşturma
    msg = MIMEMultipart()
    msg['From'] = sender_email
    msg['To'] = user_email
    msg['Subject'] = "Stock Seeker App Feedback"

    # Mesaj gövdesini ekleme
    body = f"Feedback: {feedback}\n\nFrom: {user_email if user_email else 'Anonymous'}"
    msg.attach(MIMEText(body, 'plain'))

    # SMTP sunucusuna bağlanma ve e-posta gönderme
    try:
        with smtplib.SMTP('localhost', 1025) as server:  # 1025 portunda çalışan yerel SMTP sunucusu
...
# Streamlit uygulamasındaki geri bildirim formu
st.sidebar.header("Feedback Form")
feedback_text = st.sidebar.text_area("Your Feedback")
email_input = st.sidebar.text_input("Your Email (optional)")
...
if submit_button:
    if feedback_text.strip() == '':
        ...
        if email_input.strip() != '':
            # Send email notification if email is provided
            if send_local_email(feedback_text, email_input):
                st.sidebar.write(f"A confirmation email has been sent to {email_input}.")
            else:
                st.sidebar.error("Failed to send confirmation email.")