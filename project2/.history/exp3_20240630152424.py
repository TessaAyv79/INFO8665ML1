...
def isPivot(candle, window, df):
    ...
    if candle_timestamp - timedelta(days=window)(days=window)(days=window) < df.index[0] or candle_timestamp + datetime.timedelta(days=window) >= df.index[-1]:
        return 0

    pivotHigh = 1
    pivotLow = 2
    start_index = candle_timestamp - timedelta(days=window)(days=window)(days=window)
    end_index = candle_timestamp + timedelta(days=window)(days=window)(days=window)
