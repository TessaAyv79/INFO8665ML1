...
def prepare_data(ticker, validated_period):
    try:
        # Initialize yfinance and fetch stock data
        stock_data = yf.Ticker(ticker)
        
        # Convert validated_period to string if it's not already
        if not isinstance(validated_period, str):
            validated_period = str(validated_period)
...
def handle_analysis(selected_stock, analysis_type, start_date, end_date, validate_period):
    if analysis_type != "Predicted Prices":
        display_stock_analysis(selected_stock, analysis_type, start_date, end_date, validate_period)
...
def display_stock_analysis(selected_stock, analysis_type, start_date, end_date    stock_data = yf.Ticker(selected_stock)
