...
#valid_periods = ['1y', '2y', '3y', '4y', '5y', '6y', '7y',  'max']
...
def prepare_data(selected_stock, start_date, end_date):
    ...
    return df
# def prepare_data(selected_stock, start_date, end_date):
#     # Dışardan gelen tarih bilgilerine göre veri oluştur
#     dates = pd.date_range(start=start_date, end=end_date)
    
#     # Örneğin, rastgele değerlerle dummy bir DataFrame oluştur
#     data = {
#         'Date': dates,
#         '10_day_MA': np.random.rand(len(dates)),
#         '20_day_MA': np.random.rand(len(dates)),
#         '50_day_MA': np.random.rand(len(dates)),
#         'Daily_Return': np.random.rand(len(dates))
#     }
    
#     df = pd.DataFrame(data)  # '10_day_MA' sütununu yazdırma
#     print("10_day_MA sütunu:")
#     print(df['10_day_MA'])
#     return df
...
# def train_and_evaluate_lstm_model1(selected_stock, df, company_name):
...
def handle_analysis(selected_stock, analysis_type, start_date, end_date, valid_prd):
    if analysis_type != "Predicted Prices":
        ...
        for selected_stock in selected_stocks:
            ...
            # train_and_evaluate_lstm_model1(selected_stock, df, company_name, validate_period)
            # train_and_evaluate_dnn_model(selected_stock, df, company_name)
        ...
        if checked:
            ...
            if option == "Stock Actions":
                ...
                for selected_stock in selected_stocks:
                    ...
                    # train_and_evaluate_lstm_model1(selected_stock, df, company_name)
                    # train_and_evaluate_dnn_model(selected_stock, df, company_name)
# DNN modeli eğitimi ve değerlendirme fonksiyonu
# def train_and_evaluate_dnn_model(ticker, df, company_name):
#     X = df[['10_day_MA', '20_day_MA', '50_day_MA', 'Daily_Return']]
#     y = df['Close']  # 'Adj Close' yerine 'Close' kullanıyoruz, çünkü 'Adj Close' her zaman mevcut olmayabilir
...
# proint the first few rows and the types of the columns
if submit_button:
    if feedback_text.strip() == '':
        st.sidebar.error("Feedback cannot be empty. Please provide your feedback.")
    else:
