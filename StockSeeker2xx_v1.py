...
def configure_logging(log_dir):
    ...
    # Configure logging to ensure a single log file per day
...
#     today_date = datetime.now().strftime('%Y%m%d-%H%M%S') if existing_files else today_datex
#     log_file_name = f'access_log_{today_date}.log'
#     log_file_path = os.path.join(log_dir, log_file_name)
#     logging.info('today_date = ', today_date)
#     if not os.path.exists(log_file_path):
#         open(log_file_path, 'w').close()
#         print(f'Created log file: {log_file_path}')
...
def load_and_prepare_data(stock_tickers, start_date, end_date):
    ...
    logging.info('tickers = ', stock_tickers)
    ...
    if data.empty:
        logging.error("No data available after processing all tickers.")
...
def get_company_name(selected_stock):
    try:
        # Ensure selected_stock is a string
        if isinstance(selected_stock, list):
            selected_stock = selected_stock[0]  # Take the first element if it's a list
...
def setup_streamlit():
    ...
    # Select stock tickers with a unique key
    ...
    # Define a default value or initialize valid_prd before using it
    valid_prd = None  # Default value, or initialize properly later
     
    # Assume this is where valid_prd is set based on user input or calculations
    try:
    ...
    # Select date range with unique keys
    ...
    # Analysis type with a unique key
    analysis_type = st.sidebar.selectbox("Select Analysis Type", ["Closing Prices", "Volume", "Moving Averages", "Daily Returns", "Correlation Heatmap", "Distribution of Daily Changes"], key="analysis_type")

    # Additional information options with unique keys
    selected_options = {
    ...
    st.sidebar.write("- [Morningstar Classroom](https://www.morningstar.com/learn/)")
    ...
    return data, selected_stocks, start_date, start_datex, end_date, analysis_type, selected_options, button_clicked, summary_clicked
def plot_data(data):
    try:
        fig = go.Figure()
...
def load_data(selected_stocks, start_date, end_date):
    try:
        ...
        api_url = 'your_api_endpoint_here'  # Define the API URL
...
if __name__ == "__main__":
    ...
    # Call the setup function and get the required variables
    data, selected_stocks, start_date, start_datex, end_date, analysis_type, selected_options, button_clicked, summary_clicked  = setup_streamlit()
    logging.info("Main execute")
    ...
    data_load_state = st.text('Loading data...')
    logging.info("selected ticker %s", selected_stocks)
...
def handle_analysis(selected_stock, analysis_type, start_date, end_date):
    ...
    # Ensure selected_stock is a single stock symbol as a string
    if isinstance(selected_stock, list) and len(selected_stock) > 0:
        stock_symbol = selected_stock[0]  # Extract the first element from the list
    elif isinstance(selected_stock, str):
        stock_symbol = selected_stock
    else:
        logging.error(f'Invalid selected_stock: {selected_stock}')
        stock_symbol = 'UNKNOWN'  # Default value for unknown cases

    # Convert the stock symbol to uppercase
    stock_symbol = stock_symbol.upper()  # Convert to uppercase

    logging.info(f'Selected stock: {stock_symbol}')

    if analysis_type != "Predicted Prices":
        df = data
        if df.empty:
            st.warning(f"No data available for {stock_symbol} between {start_date} and {end_date}.")
            return
        display_stock_analysis(stock_symbol, analysis_type, start_date, end_date)
        display_additional_information(stock_symbol, df)
    else:
        logging.info("Option selected: Predicted Prices")
        logging.info(f"Selected stock: {stock_symbol}")
        ...
        try:
            ...
            stock_symbol = 'APPL'  # Replace with the actual stock symbol you're working with
            ...
            # # İstek başlıkları
            # headers = {
            #     'Content-Type': 'application/json'  # JSON verileri için doğru Content-Type
            # }
            ...
            if response.status_code == 200:
                ...
                fig.update_layout(title=f'{stock_symbol} Predicted Prices',
                                  xaxis_title='Date',
                                  yaxis_title='Price')
                st.plotly_chart(fig)
                ...
                mse_rf, mae_rf, r2_rf, mape_rf, time_rf, mse_gbr, mae_gbr, r2_gbr, mape_gbr, time_gbr = train_and_evaluate_model(stock_symbol, df, stock_symbol)
                ...
                calculate_score(
                    mse_lstm=metrics['MSE'], mae_lstm=metrics['MAE'], r2_lstm=metrics['R2'], mape_lstm=metrics['MAPE'], time_lstm=metrics['Training Time'],
                    mse_rf=mse_rf, mae_rf=mae_rf, r2_rf=r2_rf, mape_rf=mape_rf, time_rf=time_rf,
                    mse_gbr=mse_gbr, mae_gbr=mse_gbr, r2_gbr=r2_gbr, mape_gbr=mape_gbr, time_gbr=time_gbr
                )
        ...
        if checked:
            ...
            if option == "Stock Actions":
                ...
                try:
                    ...
                    logging.info("Received successful response from API.")
                    #
                    # API isteğini yap ve tahminleri al
                    predictions = fetch_api_data(url, data)
                    
                    if predictions:
                        logging.info("Tahminler: %s", predictions)
                    else:
                        logging.error("Tahminler alınamadı.")
...
# Fonksiyon API'den tahminleri alır ve eksik anahtarları kontrol eder
def fetch_api_data(api_url, params):
    try:
        ...
        if 'metrics' not in data:
            logging.error('Yanıtta eksik anahtar: metrics')
            return None
