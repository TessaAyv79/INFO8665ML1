...
# Function to fetch company names from tickers
...
company_names = get_company_names(popular_tickers)
for ticker, name in zip(popular_tickers, company_names):
    print(f"{name} ({ticker})")
...
# Function to calculate moving averages and daily returns
...
def handle_nans(df, method='fill', fill_value='mean'):
    if method == 'fill':
        if fill_value == 'mean':
            df['10_day_MA'].fillna(df['10_day_MA'].mean(), inplace=True)
            df['20_day_MA'].fillna(df['20_day_MA'].mean(), inplace=True)
            df['50_day_MA'].fillna(df['50_day_MA'].mean(), inplace=True)
...
def handle_analysis(selected_stock, analysis_type, start_date, end_date):
    if analysis_type != "Predicted Prices":
        ...
        train_and_evaluate_model(selected_stock, start_date, end_date)
        train_and_evaluate_lstm_model(selected_stock, start_date, end_date)
        train_and_evaluate_dnn_model(selected_stock, start_date, end_date)
...
def display_predicted_prices(selected_stock, start_date, end_date):
    ...
    train_and_evaluate_lstm1(selected_stock, start_date, end_date)
    train_and_evaluate_model(selected_stock, start_date, end_date)
    train_and_evaluate_lstm_model(selected_stock, start_date, end_date)
    train_and_evaluate_dnn_model(selected_stock, start_date, end_date) 
...
# Function to display predicted prices
# Function to train and evaluate LSTM model 1
def train_and_evaluate_lstm1(selected_stock, start_date, end_date, prediction_days=30):
    st.subheader(f"{selected_stock} - Predicted Prices using LSTM")
    ...
    fig.update_layout(title=f'{selected_stock} Predicted Prices using LSTM',
                      xaxis_title='Date',
                      yaxis_title='Price')
    st.plotly_chart(fig)
    
# Function to train and evaluate LSTM model 1
def train_and_evaluate_model(selected_stock, start_date, end_date):
    st.subheader(f"{selected_stock} - Predicted Prices using Machine Learning Model")
    ...
    required_columns = ['10_day_MA', '20_day_MA', '50_day_MA', 'Daily_Return']
    ...
    y = df['Adj_Close']  # Adjust with your actual target variable
...
def train_and_evaluate_lstm_model(selected_stock, start_date, end_date):
    # Download historical data
    df = yf.download(selected_stock, start=start_date, end=end_date)
    ...
    st.write(f"{selected_stock} - LSTM Model Evaluation:")
    ...
    plt.legend()
    plt.title(f'Actual vs Predicted Stock Prices for {selected_stock} - LSTM')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.xticks(rotation=45)
    plt.tight_layout()
...
# Function to train and evaluate DNN model
def train_and_evaluate_dnn_model(selected_stock, start_date, end_date):
    st.subheader(f"{selected_stock} - Predicted Prices using DNN Model")
    ...
    st.write(f"{selected_stock} - DNN Model:")
    ...
    plt.legend()
    plt.title(f'Actual vs Predicted Stock Prices for {selected_stock} - DNN')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.xticks(rotation=45)
    plt.tight_layout()
