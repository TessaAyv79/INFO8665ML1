...
def display_additional_information(selected_stock, df, validate_period):
    for option, checked in selected_options.items():
        if checked:
            ...
            # Veriyi Grafik Olarak Çiz
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=prediction_dates, y=predictions, mode='lines', name='Tahmin Edilen Fiyat'))
                        fig.update_layout(title=f'{symbol} Tahmin Edilen Fiyatlar',
                              xaxis_title='Tarih',
                              yaxis_title='Fiyat')
                        st.plotly_chart(fig)
                    except KeyError as e:
                        st.error(f'Yanıtta eksik anahtar: {e}')
                    except ValueError as e:
                        st.error(f'Yanıtı işlerken hata: {e}')
                else:
                    st.error(f'Veri alınamadı, durum kodu: {response.status_code}')
                #
                display_predicted_LSTM(selected_stock, start_date, end_date)
